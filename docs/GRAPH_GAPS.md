...
- **SemanticEdge layer** for typed graph relationships (ENFORCES, INTERPRETS, SUPERSEDES, LIMITS, IMPLEMENTS,
  GUIDES, CONFLICTS).  
...
- **PCG execution gate metadata** beyond current gate result node (explicit proof obligations, irreversible
  action gating).  
