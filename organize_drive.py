...
STRATEGY_OPTIONS = [
    {
        ...
        "next_best_action": (
            "Turn on the hypervisor and execute tranche families across all planes until "
            "convergence, enforcing NONCORE → CORE promotion when risk crosses threshold."
        ),
        "super_set_generation_rules": (
            ...
            "Stop only when Δ(new_nodes,new_edges,new_terms) < EPS for N cycles and VRpt PASS "
            "for the same streak."
        ),
        "add_on_modes": [
            "@HYPERVISOR_ON",
    ...
    },
    {
        ...
        "next_best_action": (
            "Generate the Neo4j nucleus stack, then iterate until nucleus membership "
            "stabilizes for N cycles with VRpt PASS stability, focusing strata "
            "AUTHORITY, DECISIONS, ENFORCEMENT, with toroidal sharding tranche routing."
        ),
        "super_set_generation_rules": (
        ...
        "add_on_modes": [
            ...
            "@NUCLEUS_SEED",
            "@STRATUM_FOCUS=AUTHORITY,DECISIONS,ENFORCEMENT",
            "@VIEWER_OFFLINE",
            "@MANIFEST_VERIFY",
            "@SELFTEST",
            "@STRICT",
        ...
        "enterprise_spec_pattern": (
            ...
            "@DETERMINISTIC_IMPORT @NUCLEUS_SEED "
            "@STRATUM_FOCUS=AUTHORITY,DECISIONS,ENFORCEMENT @VIEWER_OFFLINE "
            "@MANIFEST_VERIFY @SELFTEST @STRICT ?OUT=ZIP+CSV+JSON+HTML+MD&ITER=auto&STRICT=true"
        ),
    },
    {
