...
def sniff_csv_schema_bytes(data: bytes, max_rows: int = 200) -> Dict[str, object]:
    ...
    return {"kind": "csv", "columns": cols, "delimiter": delim, "sample_rows": max(0, len(rows) - 1)}
        ...
        try:
            ...
            try:
                ...
                if isinstance(first, dict):
                    ...
                    keys = []
                return {"kind": "json", "mode": "jsonl", "keys": keys, "sample_lines": min(len(lines), 50)}
            except Exception:
                pass
    try:
        obj = json.loads(text)
        ...
        if isinstance(obj, list):
            ...
            return {"kind": "json", "mode": "array", "keys": keys, "sample_len": min(len(obj), 5000)}
...
def deduplicate(
    ...
    for quick, group in buckets.items():
        if len(group) == 1:
...
def write_jsonl(path: Path, objs: Iterable[Dict[str, object]]) -> int:
    ...
    with path.open("w", encoding="utf-8", newline="\n") as f:
        for o in objs:
            f.write(json.dumps(o, ensure_ascii=False, separators=(",", ":"), sort_keys=True))
            f.write("\n")
            n += 1
    return n
...
def write_markdown_binder(
    ...
    lines.append(f"- by_bucket: {json.dumps(stats.get('by_bucket'), ensure_ascii=False, separators=(',',':'))}")
    lines.append("")
...
def run(args: argparse.Namespace) -> int:
    ...
    if blueprint_zip and blueprint_zip.exists():
        ...
        ledger("blueprint_zip_extracted", {"blueprint_zip": str(blueprint_zip), "blueprint_dir": str(blueprint_dir)})
    ...
    dups_by_canon, canon_of = deduplicate(records, zip_cache, compute_full_hash=not args.dedup_quick_only)
    ledger(
        "dedup_done",
        {"canon": len(set(canon_of.values())), "dups_total": sum(len(v) for v in dups_by_canon.values())},
    )
    ...
    def record_to_verbose(rec: ArtifactRecord) -> Dict[str, object]:
        ...
        return {
            ...
            "kind": rec.kind,
            "schema": rec.schema,
            "quick_sig": {"size": rec.quick_sig[0], "head_sha1": rec.quick_sig[1], "tail_sha1": rec.quick_sig[2]},
            "sha256": rec.sha256,
            "notes": rec.notes,
        }
    ...
    if blueprint_dir:
        ...
        merged_nodes, merged_edges = merge_nodes_edges(existing_nodes, existing_edges, append_nodes, append_edges)
...
def build_argparser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(
        prog=APP_ID,
        description="Scan drives and ZIPs for Neo4j/ERD graph artifacts, dedup, build binder, and append to blueprint.",
    )
    ap.add_argument("--out-root", default=".", help="Output root directory (default: current directory).")
    ap.add_argument(
        "--roots",
        nargs="*",
        default=None,
    ...
    ap.add_argument("--seed-zips", nargs="*", default=[], help="Additional ZIP packs to scan as artifact sources.")
    ap.add_argument(
        "--blueprint-zip",
        default=None,
        help="Existing blueprint pack ZIP containing graph/neo4j_nodes.csv and graph/neo4j_edges.csv.",
    )
    ap.add_argument(
        "--model-id",
        default=None,
    ...
    ap.add_argument("--exts", default=",".join(sorted(DEFAULT_EXTS)), help="Comma-separated extension allowlist.")
    ap.add_argument("--workers", type=int, default=8, help="Worker threads for record building.")
    ap.add_argument("--follow-symlinks", action="store_true", help="Follow symlinks during scan (default: false).")
    ap.add_argument("--dedup-quick-only", action="store_true", help="Skip SHA-256 confirm step; quick signature only.")
    ap.add_argument(
        "--hash-canon-sha256",
        action="store_true",
        help="Compute SHA-256 for canonical artifacts (for stronger provenance).",
    )
    ap.add_argument("--no-vault", action="store_true", help="Disable vault copy of canonical artifacts.")
    ap.add_argument("--package-zip", action="store_true", help="Create a ZIP of the run output under out-root.")
    return ap
