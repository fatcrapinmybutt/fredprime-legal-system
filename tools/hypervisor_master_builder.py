...
# -*- coding: utf-8 -*-
"""
HYPERVISOR MASTER SUITE builder.

Builds a self-contained suite containing:
- hypervisor_master_orchestrator.py
- engines/* (adversarial, merge, apply gate, graph fuser, watcher, bucket shim)
- README_RUNBOOK.md
- RUN_MASTER.bat, WATCH_MASTER.bat
- manifest.json + zip export
"""
...
DEFAULT_BUCKET_SRC = "/mnt/data/LITIGATIONOS_DRIVE_BUCKET_FORGER_v2026_01_27D.py"
DEFAULT_TARGET = r"F:\\CAPSTONE\\Litigation_OS\\HYPERVISOR_MASTER_SUITE"


def detroit_stamp() -> str:
    utc = dt.datetime.utcnow().replace(microsecond=0)
    detroit = utc + dt.timedelta(hours=-5)
    return detroit.strftime("%Y%m%d_%H%M")
...
def file_receipt(path: Path) -> dict:
    ...
    return {
        ...
        "bytes": len(data),
        "crc32_uint32": (zlib.crc32(data) & 0xFFFFFFFF),
        "sha256": hashlib.sha256(data).hexdigest(),
    }


def load_bucket_source(bucket_src_path: Path) -> str:
    if not bucket_src_path.exists():
        raise FileNotFoundError(
            f"Bucket source not found: {bucket_src_path}. "
            "Provide --bucket-src to point at the Drive Bucket Forger source."
        )
    return bucket_src_path.read_text(encoding="utf-8", errors="replace")
...
from engines import drive_bucket_forger_shim as dbf
...
def convergence_signature(s: Dict[str, Any]) -> str:
    keep = {
        "bucket": {
            "file_count": s["bucket"].get("file_count", 0),
            "planned_actions": s["bucket"].get("planned_actions", 0),
            "zip_members": s["bucket"].get("zip_members_total", 0),
            "content_flags": s["bucket"].get("content_flags", 0),
        },
        "adversarial": {
            "events": s["adversarial"].get("events", 0),
            "hits": s["adversarial"].get("hits", 0),
        },
        "merge": {
            "merge_candidates": s["merge"].get("merge_candidates", 0),
            "merged_outputs": s["merge"].get("merged_outputs", 0),
        },
        "graph": {"nodes": s["graph"].get("nodes", 0), "edges": s["graph"].get("edges", 0)},
        "apply": {
            "applied": s["apply"].get("applied", False),
            "skipped_reason": s["apply"].get("skipped_reason", "")[:120],
        },
    }
...
def run_one_cycle(
    ...
    for d in [bucket_dir, adv_dir, merge_dir, graph_dir, apply_dir]:
        ensure_dir(d)
...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    run_id = args.run_id.strip() or (
        f"RUN_{dt.datetime.utcnow().strftime('%Y%m%dT%H%M%SZ')}_"
        f"{hashlib.sha256(os.urandom(16)).hexdigest()[:6]}"
    )
    args.run_id = run_id
    ...
    for cyc in range(1, max(1, args.cycles) + 1):
        ...
        summary = run_one_cycle(roots, cycle_dir, args, stoppers)
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        ...
        description="Hypervisor Master Orchestrator (bucket + adversarial + merge + apply gate + graph fuser + watcher).",
    )
    ...
    pr.add_argument("--adv-allow-exts", default=".txt,.md,.log,.json,.jsonl,.csv,.tsv,.html,.htm,.py,.ps1,.bat,.rtf,.docx,.pdf")
...
def should_ex_path(p: str) -> bool:
    return any(re.search(pat, p, flags=re.IGNORECASE) for pat in EXCLUDE_PATH)
...
def safe_read_text(path: Path, stoppers: List[dict]) -> str:
    try:
        ...
        stoppers.append({"code": "READ_TEXT_FAIL", "target": str(path), "detail": {"error": repr(e)}})
        try:
            return path.read_text(encoding="latin-1", errors="replace")
        except Exception as e2:
            stoppers.append(
                {"code": "READ_TEXT_FAIL_2", "target": str(path), "detail": {"error": repr(e2)}}
            )
            return ""
...
def extract_text_no_ocr(path: Path, stoppers: List[dict], max_chars: int) -> Tuple[str, str]:
    ...
    try:
        if ext in (
            ...
            t = safe_read_text(path, stoppers)
            if ext in (".html", ".htm"):
                return (strip_html_to_text(t)[:max_chars], "html")
            return (t[:max_chars], "text")
        if ext == ".docx":
            if not HAS_DOCX:
        ...
        stoppers.append({"code": "EXTRACT_FAIL", "target": str(path), "detail": {"error": repr(e)}})
        return ("", "error")
...
def find_signal_snips(text: str, bank: int, max_snips: int) -> List[Dict]:
    ...
    for idx, p in enumerate(parts):
        ...
        low = p2.lower()
        hit = False
        if bank >= 1 and any(kw in low for kw in ["not allowed", "refused", "denied", "would not let", "no chance", "not heard", "excluded", "wouldn't take evidence"]):
            hit = True
        if bank >= 2 and any(re.search(pat, low, flags=re.IGNORECASE) for pat in NEG_PATS):
            hit = True
        if bank >= 3 and any(kw in low for kw in ["ex parte", "without hearing", "without notice", "no opportunity", "reconsideration denied", "recusal denied"]):
            hit = True
        if hit:
            out.append({"idx": idx, "text": p2[:900], "mv": classify_mv(p2)})
...
FASTLANE_PATTERNS = {
    "NEGATIVE_CHARACTERIZATION": [r"\bunfit\b", r"\babusive\b", r"\bdangerous\b", r"\bunstable\b", r"\bharass", r"\bstalk", r"\bthreat", r"\bviolent\b", r"\bfraud\b", r"\bperjur", r"\bfalse\b"],
    "RIGHTS_PROCESS_VIOLATION": [r"due process", r"no notice", r"without notice", r"denied hearing", r"refused evidence", r"excluded evidence", r"not allowed", r"\bbias\b", r"\bpartial", r"ex parte", r"no opportunity"],
    "RECORD_DEFECT": [r"no transcript", r"missing transcript", r"off the record", r"not recorded", r"inaudible", r"omitted", r"record mismatch"],
}


def parse_allow_exts(s: str) -> Optional[set]:
    s = (s or "").strip()
    if not s:
        return None
...
def scan_adversarial(
    ...
    for p in [events_path, hits_path, events_csv]:
        if p.exists():
            p.unlink()

    compiled = {k: [re.compile(pat, re.IGNORECASE) for pat in pats] for k, pats in FASTLANE_PATTERNS.items()}
    with events_csv.open("w", encoding="utf-8", newline="") as fcsv:
        w = csv.DictWriter(fcsv, fieldnames=["file", "extract_mode", "mv_codes", "snippet_idx", "snippet"])
        w.writeheader()
        events = 0
        hits = 0
        for p in files:
            text, mode = extract_text_no_ocr(p, stoppers, max_chars)
            if not text.strip():
                continue
            ...
            for cat, regs in compiled.items():
                for rg in regs:
                    ...
                    if m:
                        ...
                        append_jsonl(
                            hits_path,
                            {"ts": utc_now_iso(), "file": str(p), "category": cat, "pattern": rg.pattern, "excerpt": excerpt},
                        )
                        hits += 1
                        break
...
def iter_jsonl(p: Path) -> List[dict]:
    out = []
...
def safe_read_text(path: Path, max_bytes: int) -> str:
    try:
        b = safe_read_bytes(path, max_bytes)
...
def canonical_choice(paths: List[str], meta: Dict[str, Any]) -> str:
    ...
    for p in paths:
        m = meta.get(p, {}) or {}
        key = (int(m.get("mtime_epoch", 0)), int(m.get("size_bytes", 0)))
        if key > best_key:
            best_key = key
            best = p
...
def merge_text_block(sources: List[Dict[str, Any]], max_bytes_per_file: int) -> str:
    ...
    for src in sources:
        path = Path(src["path"])
        txt = safe_read_text(path, max_bytes_per_file)
        parts.append(
            "\n".join(
                [
                    "===== SOURCE_BEGIN =====",
...
def merge_json_smart(sources: List[Dict[str, Any]], max_bytes_per_file: int) -> Tuple[str, str]:
    ...
    for src in sources:
        path = Path(src["path"])
        txt = safe_read_text(path, max_bytes_per_file).strip()
        if not txt:
            continue
        ext = path.suffix.lower()
        if ext in (".jsonl", ".ndjson"):
            for ln in txt.splitlines():
                ...
                if not ln:
                    continue
                try:
                    items.append({"source": src["path"], "row": json.loads(ln)})
                except Exception:
                    items.append({"source": src["path"], "row_raw": ln})
        else:
            try:
                obj = json.loads(txt)
                items.append({"source": src["path"], "json": obj})
            except Exception:
                items.append({"source": src["path"], "raw": txt})
...
def run_merge_pipeline(
    ...
    for r in recs:
        try:
            meta[r["path"]] = {
                ...
                "mtime": r.get("mtime", ""),
                "mtime_epoch": r.get("mtime_epoch", 0),
                "ext": r.get("ext", ""),
            }
        except Exception:
            continue
    ...
    for fam_key, fam in (versions or {}).items():
        ...
        if ext in TABLE_EXT and enable_table_merge:
            ...
            provenance["skipped"].append({"family": fam_key, "reason": "unsupported_or_disabled", "canonical": canon, "sources": paths})
            append_jsonl(plan_path, {"action": "MERGE_SKIPPED", "family": fam_key, "reason": "unsupported_or_disabled", "canonical": canon, "sources": paths})
            continue

        merged_outputs += 1
        provenance["merged_outputs"].append({"family": fam_key, "method": method, "output": str(out_path), "canonical": canon, "sources": paths})
        append_jsonl(plan_path, {"action": "MERGE_CREATE", "family": fam_key, "method": method, "output": str(out_path), "canonical": canon, "sources": paths})
...
def apply_gate_and_maybe_execute(
    ...
    if planned_actions_jsonl and planned_actions_jsonl.exists():
        ...
        stoppers.append({"code": "APPLY_PLAN_MISSING", "target": str(planned_actions_jsonl), "detail": {"note": "No plan file to compute token."}})
        token = ""

    if not destructive:
        if bucket_apply == "copy" and (not policy_allow_copy_without_confirm):
            if confirm_token.strip() != token:
                return {"applied": False, "skip_apply_execution": True, "skipped_reason": "COPY_GATED_TOKEN_MISMATCH", "token": token}
    ...
    if (not confirm_token) or (confirm_token.strip() != token):
        return {"applied": False, "skip_apply_execution": True, "token": token, "skipped_reason": "DESTRUCTIVE_APPLY_REQUIRES_CONFIRM_TOKEN"}
...
"""
GRAPH_FUSER
Builds a combined graph pack from bucket + adversarial + merge outputs.
"""
from __future__ import annotations
...
def fuse_graph(
        ...
        try:
            recs.append(
                dbf.FileRecord(
                    ...
                    mtime_epoch=float(r.get("mtime_epoch", 0)),
                    ext=str(r.get("ext", "")),
                    bucket=str(r.get("bucket", "")),
                    sha256=str(r.get("sha256", "")),
                    quick_sig=str(r.get("quick_sig", "")),
                    version_family=str(r.get("version_family", "")),
                    integrity_key=str(r.get("integrity_key", "")),
                )
            )
        except Exception as e:
            stoppers.append({"code": "GRAPH_REC_PARSE_FAIL", "target": str(r.get("path", "")), "detail": {"error": repr(e)}})

    actions_raw = iter_jsonl(bucket_plan_jsonl)
    actions = []
    for a in actions_raw:
        try:
            actions.append({"src": a.get("src", ""), "dst": a.get("dst", ""), "op": a.get("op", "")})
        except Exception:
            continue
    ...
    if merge_provenance_json and merge_provenance_json.exists():
        try:
            ...
            for mo in prov.get("merged_outputs", []):
                outp = mo.get("output", "")
                if outp:
                    merge_nodes.append({"id": "MERGED:" + outp, "label": "MergedArtifact", "path": outp})
                    for src in mo.get("sources", []):
                        merge_edges.append({"src": "FILE:" + src, "dst": "MERGED:" + outp, "type": "MERGED_INTO"})
        except Exception as e:
            stoppers.append({"code": "MERGE_PROV_PARSE_FAIL", "target": str(merge_provenance_json), "detail": {"error": repr(e)}})
    ...
    for n in nodes:
        ...
        if not nid:
            continue
        if nid not in uniq_nodes:
            uniq_nodes[nid] = n
    nodes = list(uniq_nodes.values())

    for n in nodes:
        append_jsonl(nodes_path, n)
    for e in edges:
        append_jsonl(edges_path, e)

    node_fields = sorted(list(set().union(*[set(n.keys()) for n in nodes])))
    edge_fields = sorted(list(set().union(*[set(e.keys()) for e in edges])))
    ...
    try:
        ...
        stoppers.append({"code": "VIRTUAL_GRAPH_FAIL", "target": str(out_dir / "virtual_graph.sqlite"), "detail": {"error": repr(e)}})
    try:
        positions = {}
        dbf.emit_html_graph_viewer(out_dir / "graph_viewer.html", nodes, edges, positions, title=f"Graph — {run_id}")
    except Exception as e:
        stoppers.append({"code": "HTML_GRAPH_FAIL", "target": str(out_dir / "graph_viewer.html"), "detail": {"error": repr(e)}})
    try:
        ...
        stoppers.append({"code": "DASHBOARD_FAIL", "target": str(out_dir / "dashboard_index.html"), "detail": {"error": repr(e)}})
...
def hash_id(ev: dict) -> str:
    s = json.dumps(ev, sort_keys=True, ensure_ascii=False).encode("utf-8")
...
from . import drive_bucket_forger_shim as dbf
...
def snapshot(roots: List[Path], max_files: int) -> Dict[str, Tuple[int, int]]:
    snap = {}
    for p in iter_files_poll(roots, max_files):
        try:
            st = p.stat()
            snap[str(p)] = (int(st.st_size), int(st.st_mtime))
        except Exception:
            continue
    return snap
...
def watch_loop(
    ...
    while True:
        ...
        d = diff_snap(last_snap, cur)
        last_snap = cur
        any_change = bool(d["added"] or d["changed"] or d["removed"])
        if any_change:
            append_jsonl(events, {"ts": utc_now_iso(), "event": "FS_CHANGE", "diff": {k: len(v) for k, v in d.items()}})
            now = time.time()
            if (now - last_fire) < debounce_seconds:
                append_jsonl(events, {"ts": utc_now_iso(), "event": "DEBOUNCE_SKIP", "seconds_since_last": (now - last_fire)})
                continue
            last_fire = now
            run_id = f"WATCHRUN_{dt.datetime.utcnow().strftime('%Y%m%dT%H%M%SZ')}_{hashlib.sha256(os.urandom(8)).hexdigest()[:6]}"
            write_json(last_trigger, {"ts": utc_now_iso(), "run_id": run_id, "diff_counts": {k: len(v) for k, v in d.items()}})
            ...
            adv_res = ase.scan_adversarial(
                ...
                allow_exts=getattr(trigger_args, "adv_allow_exts", ".txt,.md,.log,.json,.jsonl,.csv,.tsv,.html,.htm,.py,.ps1,.bat,.rtf,.docx,.pdf"),
                max_files=10000,
                max_chars=2000000,
                stoppers=stoppers,
...
BUCKET_SHIM_SRC = r'''#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
drive_bucket_forger shim
Provides bucket_cycle(...) using functions from drive_bucket_forger.
"""
from __future__ import annotations
...
from . import drive_bucket_forger as core
...
def bucket_cycle(
    ...
    zip_inventory: bool,
    zip_hash_members: bool,
    ocr_queue: bool,
    content_scan: bool,
    content_terms_file: str,
    content_extra_terms: List[str],
    content_include_exts: str,
    content_max_bytes: int,
    content_max_terms: int,
    plan_cleanup: bool,
    apply_cleanup: bool,
    halt_on_hard: bool,
    apply_dir: Path,
    ...
    bumpers: List[core.Bumper] = []

    recs = []
    count = 0
    for fr in core.iter_files(roots):
        recs.append(fr)
        count += 1
        ...
        for r in recs:
            try:
                if (not r.sha256) or verify_existing_hash:
                    r.sha256 = core.sha256_file(Path(r.path))
            except Exception as e:
                stoppers.append({"code": "HASH_FAIL", "target": str(r.path), "detail": {"error": repr(e)}})
    if quick_sig:
        for r in recs:
            try:
                r.quick_sig = core.quick_signature(Path(r.path))
            except Exception as e:
                stoppers.append({"code": "QUICK_SIG_FAIL", "target": str(r.path), "detail": {"error": repr(e)}})

    for r in recs:
        try:
            r.integrity_key = core.integrity_key(Path(r.path))
            r.bucket = core.classify_bucket(Path(r.path))
            r.version_family = core.detect_version_family(Path(r.path))
        except Exception as e:
            stoppers.append({"code": "RECORD_ENRICH_FAIL", "target": str(r.path), "detail": {"error": repr(e)}})
    ...
    actions = core.plan_bucket_paths(Path(bucket_out_root), recs, bumpers, copy_mode, hash_on, verify_existing_hash)
    ...
    with plan_moves.open("w", encoding="utf-8", newline="\n") as f:
        for a in actions:
            f.write(
                json.dumps({"op": a.op, "src": str(a.src), "dst": str(a.dst), "reason": a.reason}, ensure_ascii=False)
                + "\n"
            )
    ...
    if (not plan_only) and (not copy_mode):
        ...
        try:
            apply_counts = core.apply_plan(actions, bumpers, True, apply_dir, halt_on_hard)
            apply_executed = True
        except Exception as e:
            stoppers.append({"code": "COPY_APPLY_FAIL", "target": str(apply_dir), "detail": {"error": repr(e)}})

    dups = core.dedupe_groups(recs)
    dups_sum = core.build_duplicates_summary(dups)
    dup_json = out_dir / "duplicates_summary.json"
    dup_json.write_text(json.dumps(dups_sum, indent=2, ensure_ascii=False) + "\n", encoding="utf-8", newline="\n")

    fams = core.group_versions(recs)
    ver_sum = core.build_versions_summary(fams)
        ...
        with zip_inventory_jsonl.open("w", encoding="utf-8", newline="\n") as f:
            for r in recs:
                if str(r.ext).lower() == ".zip":
                    try:
                        inv = core.inventory_zip(Path(r.path), bumpers, hash_members=zip_hash_members)
                        for item in inv:
                            item["_zip_path"] = str(r.path)
                            f.write(json.dumps(item, ensure_ascii=False) + "\n")
                        zip_members_total += len(inv)
                    except Exception as e:
                        stoppers.append({"code": "ZIP_INV_FAIL", "target": str(r.path), "detail": {"error": repr(e)}})
        ...
        with ocr_jsonl.open("w", encoding="utf-8", newline="\n") as f:
            for r in recs:
                if str(r.ext).lower() == ".pdf":
                    try:
                        if core.needs_ocr_pdf(Path(r.path)):
                            f.write(json.dumps({"path": str(r.path), "reason": "needs_ocr_pdf_heuristic"}, ensure_ascii=False) + "\n")
                    except Exception as e:
                        stoppers.append({"code": "OCR_HEUR_FAIL", "target": str(r.path), "detail": {"error": repr(e)}})
    ...
    if content_scan:
        try:
            ...
            _summary, flags, adversarial_hits = core.content_scan_and_adversarial(
                recs,
                bumpers,
            ...
            with adv_hits_path.open("w", encoding="utf-8", newline="\n") as f:
                for it in adversarial_hits:
                    f.write(json.dumps(it, ensure_ascii=False) + "\n")
        except Exception as e:
            stoppers.append({"code": "CONTENT_SCAN_FAIL", "target": str(out_dir), "detail": {"error": repr(e)}})

    cleanup_plan_json = out_dir / "cleanup_plan.json"
    if plan_cleanup:
        try:
            plan = core.plan_empty_folder_cleanup(roots, bumpers)
            cleanup_plan_json.write_text(json.dumps(plan, indent=2, ensure_ascii=False) + "\n", encoding="utf-8", newline="\n")
        except Exception as e:
            stoppers.append({"code": "CLEANUP_PLAN_FAIL", "target": str(out_dir), "detail": {"error": repr(e)}})
    ...
    with bumpers_jsonl.open("w", encoding="utf-8", newline="\n") as f:
        for b in bumpers:
            f.write(json.dumps({"code": b.code, "severity": b.severity, "target": b.target, "detail": b.detail}, ensure_ascii=False) + "\n")
...
RUNBOOK_TEMPLATE = """# RUNBOOK — HYPERVISOR MASTER SUITE ({timestamp} America/Detroit)
...
def build_suite_files(bucket_src: str, builder_name: str, timestamp: str) -> Dict[str, str]:
    runbook = RUNBOOK_TEMPLATE.format(timestamp=timestamp, builder_name=builder_name)
    return {
        "SUITE/hypervisor_master_orchestrator.py": ORCHESTRATOR_SRC,
        "SUITE/engines/__init__.py": "from . import drive_bucket_forger\n",
        "SUITE/engines/drive_bucket_forger.py": bucket_src,
        "SUITE/engines/drive_bucket_forger_shim.py": BUCKET_SHIM_SRC,
        ...
        "SUITE/README_RUNBOOK.md": runbook,
    }


def build_launchers(target: Path) -> None:
    run_bat = "\r\n".join(
        [
            "@echo off",
            "setlocal",
...
def build_manifest(target: Path, written: list[Path]) -> None:
    manifest = {"files": [], "count": 0}
    for p in written:
        manifest["files"].append(file_receipt(p))
    manifest["count"] = len(manifest["files"])
    write_text(target / "manifest.json", json.dumps(manifest, indent=2))


def build_zip(target: Path, suite_files: Dict[str, str], zip_name: str) -> Path:
    ...
    with zipfile.ZipFile(zip_path, "w", compression=zipfile.ZIP_DEFLATED, compresslevel=6) as z:
        ...
        z.write(target / "RUN_MASTER.bat", arcname="RUN_MASTER.bat")
        z.write(target / "WATCH_MASTER.bat", arcname="WATCH_MASTER.bat")
        z.write(target / "manifest.json", arcname="manifest.json")
    return zip_path
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Build HYPERVISOR_MASTER_SUITE to a durable target folder.")
    parser.add_argument("--bucket-src", default=DEFAULT_BUCKET_SRC, help="Path to Drive Bucket Forger source.")
    parser.add_argument("--target", default=DEFAULT_TARGET, help="Target output folder.")
    args = parser.parse_args()

    bucket_src = load_bucket_source(Path(args.bucket_src))
    suite_files = build_suite_files(bucket_src, builder_name, dt.datetime.utcnow().strftime("%Y-%m-%d %H:%M"))
    ...
    build_manifest(target, written)
    written.append(target / "manifest.json")

    zip_path = build_zip(target, suite_files, zip_name)

    print("BUILT:", str(target))
    print("RUN:", str(target / "RUN_MASTER.bat"))
    print("WATCH:", str(target / "WATCH_MASTER.bat"))
