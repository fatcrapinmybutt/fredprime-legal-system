...
def run_cyclepack() -> int:
    ...
    for record in manifest_inputs:
        ...
        if record.get("zip_openable") and path.suffix.lower() == ".zip":
            ...
            try:
                ...
                archives.append(
                    {"archive": record["path"], "extracted_to": safe_relpath(target, out_root), "members": len(members)}
                )
    ...
    with open(scripts_catalog_csv, "w", encoding="utf-8", newline="") as handle:
        ...
        for record in sorted(script_rows, key=lambda x: x["path"]):
            writer.writerow(
                [record["path"], record["ext"], record["size_bytes"], record["sha256"] or "", record["crc32"] or "", record["plane_id"]]
            )
        ...
        if not blockers:
            ...
            if incomplete:
                ...
                for blocker in incomplete:
                    ...
                    handle.write(
                        "  - Provide **all parts** belonging to this split set (same base name). If possible, upload the reconstructed full `.zip` instead.\n"
                    )
                    handle.write(
                        "  - After upload: rerun the builder script; it will attempt open + extract and will record `zip_openable=true` only if it passes.\n"
                    )
            else:
...
""",
    ...
    ledger(
        cycle_ledger,
        {"event": "cycle_complete", "run_id": run_id, "cycle": 3, "status": "PASS_WITH_BLOCKERS" if blockers else "PASS", "delta_eps_est": 0.0},
    )
...
if __name__ == "__main__":
    raise SystemExit(run_cyclepack())
