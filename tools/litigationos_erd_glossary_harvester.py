...
def determine_blueprint_target(
    blueprint_root: Path,
    role: str,
    ext: str,
    content_hash: str,
...
class StateDB:
    ...
    def get_occurrence(self, origin_path: str) -> Optional[Dict[str, object]]:
        cur = self.conn.execute(
            "SELECT origin_path,size_bytes,mtime_utc,quick_fp,content_hash,artifact_id,last_seen_ts FROM occurrences WHERE origin_path=?",
            (origin_path,),
        )
        row = cur.fetchone()
        if not row:
    ...
    def get_artifact(self, content_hash: str) -> Optional[Dict[str, object]]:
        cur = self.conn.execute(
            "SELECT content_hash,artifact_id,ext,role,bucket,bytes,blueprint_path,first_seen_ts,last_seen_ts FROM artifacts WHERE content_hash=?",
            (content_hash,),
        )
        row = cur.fetchone()
        if not row:
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    ...
    p.add_argument(
        "--include-media",
        action="store_true",
        help="Also index media assets (pdf/png/jpg/etc). Default off.",
    )
    p.add_argument(
        "--exclude-dir",
        nargs="*",
    ...
    p.add_argument(
        "--out-root",
        default=str(Path.cwd() / "OUT_ERD_GLOSSARY_HARVEST"),
    ...
    for cycle in range(1, total_cycles + 1):
        ...
        for hit in iter_candidate_files(
            ...
            try:
                ...
                artifact_id = (
                    stable_artifact_id(content_hash)
                    if content_hash
                    else f"ARTQF_{hashlib.blake2b(qf.encode('utf-8', errors='ignore'), digest_size=10).hexdigest()}"
                )
                ext = hit.ext
                ...
                existing_art = run_artifacts.get(artifact_key) or state_db.get_artifact(content_hash) if content_hash else None
        ...
        if args.watch:
            ...
            if prior_unique_artifacts is not None and current_unique_artifacts == prior_unique_artifacts and counters["uniques"] == 0:
                stable_cycles += 1
            else:
                stable_cycles = 0
