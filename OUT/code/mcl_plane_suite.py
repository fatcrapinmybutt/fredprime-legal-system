...
def sha256_file(path: Path, bumper: "BumperLog", chunk_size: int = 1024 * 1024) -> str:
    try:
        h = hashlib.sha256()
        ...
        bumper.add("HASH_FAIL", str(path), {"error": repr(e)})
        return ""
...
def write_bucket_inventory(files: List[Path], registry: Dict, roots: List[Path], out_csv: Path) -> None:
    ...
    with out_csv.open("w", encoding="utf-8", newline="") as f:
        ...
        for p in files:
            ...
            try:
                st = p.stat()
                mtime = _dt.datetime.fromtimestamp(st.st_mtime, tz=_dt.timezone.utc).replace(microsecond=0).isoformat().replace("+00:00", "Z")
                size = st.st_size
            except Exception:
                mtime, size = "", ""
...
def adversarial_scan_files(files: List[Path], scan_roots: List[Path], out_dir: Path, cfg: Dict, bumper: BumperLog) -> Dict:
    ...
    with findings_csv.open("w", encoding="utf-8", newline="") as fcsv:
        w = csv.writer(fcsv)
        w.writerow(["finding_id", "mv_code", "category", "weight", "pattern", "location", "offset_start", "offset_end", "path", "snippet", "snippet_hash"])
        for fp in files:
            file_count += 1
            ...
            for s in sentences:
                ...
                for cat, weight, rx in compiled:
                    ...
                    finding = Finding(
                        ...
                        location=loc,
                        offset_start=cum,
                        offset_end=cum + len(s),
                        category=cat,
                        mv_code=mv,
                        weight=float(weight),
                        pattern=rx.pattern,
                        snippet=snip[:1000],
                        snippet_hash=sha256_hex(snip.encode("utf-8", errors="replace")),
                        created_utc=UTCNOW(),
                    )
                    findings.append(finding)
                    append_jsonl(findings_jsonl, dataclasses.asdict(finding))
                    w.writerow([fid, mv, cat, weight, rx.pattern, loc, cum, cum + len(s), str(fp), snip[:500], sha256_hex(snip.encode("utf-8", errors="replace"))])
...
def adversarial_converge(files: List[Path], scan_roots: List[Path], run_root: Path, base_cfg: Dict,
    ...
    with con_csv.open("w", encoding="utf-8", newline="") as f:
        w = csv.writer(f)
        w.writerow(["finding_id", "mv_code", "category", "weight", "pattern", "location", "offset_start", "offset_end", "path", "snippet"])
        for fid in sorted(all_findings.keys()):
            fnd = all_findings[fid]
            w.writerow([fnd.finding_id, fnd.mv_code, fnd.category, fnd.weight, fnd.pattern, fnd.location, fnd.offset_start, fnd.offset_end, fnd.file_path, fnd.snippet])
...
def neo4j_export(run_root: Path, files: List[Path], scan_roots: List[Path], adv_summary: Dict, cfg: Dict, evidence_index: Dict[str, Dict]) -> None:
    ...
    with files_csv.open("w", encoding="utf-8", newline="") as f:
        w = csv.writer(f)
        w.writerow(["file_id:ID", "path", "relpath", "ext", "bucket", "size_bytes:long", "mtime_utc", "sha256"])
        ...
        for p in files:
            ...
            for r in scan_roots:
                if str(p).lower().startswith(str(r).lower()):
                    ...
                    break
            fid = stable_id(str(p))
            sha = evidence_index.get(str(p), {}).get("sha256", "")
            w.writerow([fid, str(p), rel, ext, bucket, size, mtime, sha])
    ...
    with findings_csv.open("w", encoding="utf-8", newline="") as f_nodes, \
        ...
        wn.writerow(["finding_id:ID", "category", "mv_code", "weight:float", "pattern", "location", "offset_start:long", "offset_end:long", "snippet", "snippet_hash", "created_utc", "file_path"])
        ...
        for line in consolidated.read_text(encoding="utf-8", errors="replace").splitlines():
            ...
            wn.writerow([d.get("finding_id"), d.get("category"), d.get("mv_code"), d.get("weight"),
                         d.get("pattern"), d.get("location"), d.get("offset_start"), d.get("offset_end"),
                         d.get("snippet"), d.get("snippet_hash"), d.get("created_utc"), d.get("file_path")])
...
def converge(run_root: Path, in_root: Path, out_root: Path, scan_roots: List[Path],
    ...
    evidence_index = build_evidence_index(files, run_dir / "RUN" / "evidence_index.jsonl", bumper, hash_files)
    ...
    if adversarial:
        adv_cfg = load_adversarial_config(adv_cfg_path, bumper)
        neo4j_export(run_dir, files, scan_roots, best_summary, adv_cfg, evidence_index)
    ...
    write_run_manifest(run_dir, in_root, out_root, scan_roots, files, best_summary, web_enabled, adversarial, hash_files, excludes, max_files)

    return final


def build_evidence_index(files: List[Path], out_path: Path, bumper: BumperLog, hash_files: bool) -> Dict[str, Dict]:
    ...
    with out_path.open("w", encoding="utf-8", newline="\n") as f:
        for p in files:
            try:
                ...
                mtime, size = "", ""
            sha = sha256_file(p, bumper) if hash_files else ""
            row = {
                "path": str(p),
                "size_bytes": size,
                "mtime_utc": mtime,
...
def write_run_manifest(run_root: Path, in_root: Path, out_root: Path, scan_roots: List[Path], files: List[Path],
                       adv_summary: Dict, web_enabled: bool, adversarial: bool, hash_files: bool,
                       excludes: List[str], max_files: int) -> None:
    manifest = {
        "generated_utc": UTCNOW(),
        ...
        "features": {
            ...
            "web_harvest": web_enabled,
            "hash_files": hash_files,
        },
        "counts": {
            "files_seen": len(files),
            "findings_total": adv_summary.get("total_findings", 0),
        },
        "outputs": {
            "inventory_csv": str(run_root / "INVENTORY" / "doctype_bucket_inventory.csv"),
            "evidence_index": str(run_root / "RUN" / "evidence_index.jsonl"),
            "adversarial_summary": str(run_root / "ADVERSARIAL" / "CONSOLIDATED" / "summary.json") if adversarial else "",
        },
    }
    write_json(run_root / "RUN" / "run_manifest.json", manifest)
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    ...
    p.add_argument("--hash-files", type=str, default="true", help="Hash files for evidence index (true/false)")
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    converge(
        ...
        hash_files=boolish(args.hash_files),
    )
