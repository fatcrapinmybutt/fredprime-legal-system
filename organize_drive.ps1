    ...
    try {
        ...
        if (-not [string]::IsNullOrWhiteSpace($RunManifestPath)) {
            $manifest = [pscustomobject]@{
                ...
                strategy_options  = @(
                    [pscustomobject]@{
                        option                    = 'Hypervisor: full-plane tranche convergence'
                        next_best_action          = 'Turn on the hypervisor and execute tranche families across all planes until convergence, enforcing NONCORE → CORE promotion when risk crosses threshold.'
                        super_set_generation_rules = 'Every cycle emits RUN_LEDGER.jsonl, MANIFEST.json, DELTA_SUMMARY.md, STRATUM_METRICS.csv, VRpt.md, TRANCHE_RUNS.csv, PARALLEL_TRACK_STATUS.json. Stop only when Δ(new_nodes,new_edges,new_terms) < EPS for N cycles and VRpt PASS for the same streak.'
                        add_on_modes              = @('@HYPERVISOR_ON','@AUTONOMY_MAX','@SHARD_BY_DOC','@BACKPRESSURE','@MULTIMODAL_POOLS','@PROMOTE_NONCORE_TO_CORE','@STRICT','@REPLAYABLE_RUN','@CONVERGENCE_EPS','@VRPT_PASS_STREAK','@TOROIDAL_SHARDING')
                        enterprise_spec_pattern   = 'EXPLODE_SUPERPIN:HYPERVISOR @HYPERVISOR_ON @AUTONOMY_MAX @TOROIDAL_SHARDING @BACKPRESSURE @PROMOTE_NONCORE_TO_CORE @STRICT ?EPS=0.005&N=3&OUT=ZIP+MD+CSV+JSON&ITER=auto&STRICT=true'
                    }
                    [pscustomobject]@{
                        option                    = 'Neo4j nucleus: schema contract + constraints-first + deterministic import + offline viewer'
                        next_best_action          = 'Generate the Neo4j nucleus stack, then iterate until nucleus membership stabilizes for N cycles with VRpt PASS stability, focusing strata AUTHORITY, DECISIONS, ENFORCEMENT, with toroidal sharding tranche routing.'
                        super_set_generation_rules = 'Every cycle emits schema_contract.json, constraints.cypher, import.cypher, split nodes.csv/edges.csv, nucleus/seeds.json, viewer/index.html, plus RUN_LEDGER.jsonl, MANIFEST.json, DELTA_SUMMARY.md, STRATUM_METRICS.csv, VRpt.md. Stop only when nucleus membership stable N cycles and VRpt PASS N cycles.'
                        add_on_modes              = @('@NEO4J_NUCLEUS','@SCHEMA_LOCK','@CONSTRAINTS_FIRST','@DETERMINISTIC_IMPORT','@NUCLEUS_SEED','@STRATUM_FOCUS=AUTHORITY,DECISIONS,ENFORCEMENT','@VIEWER_OFFLINE','@MANIFEST_VERIFY','@SELFTEST','@STRICT','@TOROIDAL_SHARDING')
                        enterprise_spec_pattern   = 'EXPLODE_SUPERPIN:GRAPH @NEO4J_NUCLEUS @SCHEMA_LOCK @CONSTRAINTS_FIRST @DETERMINISTIC_IMPORT @NUCLEUS_SEED @STRATUM_FOCUS=AUTHORITY,DECISIONS,ENFORCEMENT @VIEWER_OFFLINE @MANIFEST_VERIFY @SELFTEST @STRICT ?OUT=ZIP+CSV+JSON+HTML+MD&ITER=auto&STRICT=true'
                    }
                    [pscustomobject]@{
                        option                    = 'Forms-first Vehicle Router: Relief → Form → Standard → Elements → POs → Deadlines → Service → Exhibits'
                        next_best_action          = 'Execute Forms-First Vehicle Router end-to-end with PO promotion logic enabled, fail-closed if any CORE obligations, deadlines, service, or VRpt are uncertain.'
                        super_set_generation_rules = 'Every cycle emits VehicleMap.md, PO_DB.csv, Deadlines.csv, ServicePlan.md, ExhibitMatrix.csv, plus RUN_LEDGER.jsonl, MANIFEST.json, DELTA_SUMMARY.md, STRATUM_METRICS.csv, VRpt.md. Packaging blocked if any CORE PO is OPEN or PARTIAL.'
                        add_on_modes              = @('@PCW','@FORMSFIRST','@VEHICLE_MAP','@PO_DB_BUILD','@DEADLINE_ENGINE','@SERVICE_CHAIN','@EXHIBIT_MATRIX','@QUOTELOCK','@PROMOTE_NONCORE_TO_CORE','@FAIL_CLOSED','@STRICT')
                        enterprise_spec_pattern   = 'EXPLODE_SUPERPIN:FORMS @PCW @FORMSFIRST @VEHICLE_MAP @PO_DB_BUILD @DEADLINE_ENGINE @SERVICE_CHAIN @EXHIBIT_MATRIX @PROMOTE_NONCORE_TO_CORE @FAIL_CLOSED @STRICT ?PIPE=RELIEF>FORM>STANDARD>ELEMENTS>PO>DEADLINES>SERVICE>EXHIBITS&OUT=ZIP+MD+CSV+JSON&ITER=auto&STRICT=true'
                    }
                )
            }
