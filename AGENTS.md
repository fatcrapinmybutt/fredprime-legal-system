...
- Use branches in the form `codex/<feature>`. Branch names containing `core`,
  `engine`, `matrix`, `echelon`, `patch`, or `hotfix` trigger the full test
  suite.
- Documentation-only changes skip tests. Any logic modification runs `black`,
  `mypy --strict`, `flake8`, and all unit tests under `tests/`.
- Commit messages follow the `[type] message` pattern.

### Required Directories

The project expects these folders:

`core/`, `modules/`, `gui/`, `cli/`, `config/`, `docs/`, `tests/`

Maintain this layout when adding new components.

### Runtime Hooks

- Changing `codex_brain.py` rebuilds dependency maps.
- Editing any GUI file triggers layout snapshot tests.
- Commits on a patch branch must run `codex_patch_manager.py` to inject the
  change into the current build.
