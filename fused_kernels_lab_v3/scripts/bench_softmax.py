...
def main() -> int:
    ...
    def eager_ref():
        ...
        return torch.softmax(x.masked_fill(~mask, float("-inf")), dim=-1)
    ...
    for _ in range(args.warmup):
        fused_masked_softmax(x, mask).y
    _sync()
    t0 = time.perf_counter()
    used = 0
    for _ in range(args.iters):
        r = fused_masked_softmax(x, mask)
        used += int(r.used_triton)
    _sync()
    t1 = time.perf_counter()
    fused_ms = (t1 - t0) * 1000.0 / args.iters

    print("Benchmark: masked softmax (row-wise)")
    print(f"M={args.m} N={args.n} dtype={dtype} mask_rate={args.mask_rate}")
